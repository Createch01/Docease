/* DocEase — contact icon set.
   ...
   inline SVG on a 24×24 grid.
*/

import React from 'react';

export const RX_ICON_KEYS = ['name', 'address', 'location', 'phone', 'mail', 'website', 'cellphone', 'fax'] as const;
export type RxIconKey = (typeof RX_ICON_KEYS)[number];

export const RX_ICON_LABELS: Record<string, string> = {
  name: 'Nom',
  address: 'Adresse',
...
/* Each entry: { outline: (sw: number) => React.ReactNode, filled: () => React.ReactNode } */
export const RX_ICON_PATHS: Record<string, { outline: (sw: number) => React.ReactNode; filled: () => React.ReactNode }> = {
  name: {
    outline: (sw: number) => (
...
export const RxIcon: React.FC<RxIconProps> = ({
  name = 'phone',
  style = 'outline',
  size = 16,
  color = 'currentColor',
